...
// stone has ported this to Raspberry Pi based on WringPi.
#include "oled.h"
#include <unistd.h>
...
void LCD_WrDat(u8 data)
{
	u8 i=8;
	//LCD_CS=0;;
	digitalWrite(CMD_PIN,1);//CMD =1 :write data 
	wiringPiSPIDataRW(0,&data,1);
}
void LCD_WrCmd(u8 cmd)
{
	u8 i=8;
	digitalWrite(CMD_PIN,0);//CMD= 0; write command
...
void LCD_Init(void)        
{
	...
	// start wiringPi interface
	wiringPiSetup();
	
	// initialize SPI interface 0 
	wiringPiSPISetup(0,5000000);
	
	
	// set CMD line (CS)
	pinMode(CMD_PIN,OUTPUT);
	pinMode(RST_PIN,OUTPUT);//Reset PIN initial 
	
	
	digitalWrite(CMD_PIN,1);//disable cmd now
	digitalWrite(RST_PIN,0);//generate RST signal 
	usleep(50000);//keep rst pin 50ms low level 
	digitalWrite(RST_PIN,1);//finish reset , go into normal mode 
...
void LCD_Rectangle(u8 x1,u8 y1,u8 x2,u8 y2,u8 gif)
{
	...
	{
		LCD_WrDat(0x01<<(y1%8)); 			
		if(gif == 1) 	usleep(50000);
	}  
	LCD_Set_Pos(x1,y2>>3);
  for(n=x1;n<=x2;n++)
	{
		LCD_WrDat(0x01<<(y2%8)); 			
		if(gif == 1) 	usleep(5000);
	}
...
//返回：无
//==============================================================  
void LCD_P6x8Str(u8 x,u8 y,u8 *ch,const u8 *sF6x8)
{
	u8 c=0,i=0,j=0;
...
// clear  one line 16 lines 
// print  string on that line
// line = 0,1,2,3
void PrintLine(u8 line, u8 * ch)
{
   u8 i = 0; 
   // only 4 lines, not support line exceeding 3
   if (line > 3)
   	return;
   ...
   LCD_P8x16Str(0,16*line, ch, F8X16);
}
